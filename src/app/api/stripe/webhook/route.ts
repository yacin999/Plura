...
export async function POST(req:NextRequest) {
    ...
    try {
        ...
        if (!sig || !webhookSecret) {
            console.log("Error : webhook secret or signature does not exist.")
            return
        }
    ...
    try {
        if (stripeWebhookEvents.has(stripeEvent.type)) {
            ...
            if (!subscription.metadata.connectAccountPayments && !subscription.metadata.connectAccountSubscriptions) {
                ...
                console.log("SKIPPED FROM WEBHOOK because subscription was from a connected account not from the application")
            }
        }
    } catch (error) {
        console.log("error")
        return new NextResponse("Web Rrror", {status : 400})
    }
