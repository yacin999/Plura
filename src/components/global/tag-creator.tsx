...
import { v4 } from 'uuid'
import { deleteTag, getTagsForSubaccount, saveActivityLogsNotification, upesetTag } from '@/lib/queries'
...
const TagCreator = ({subAccountId, getSelectedTags, defaultTags}: Props) => {
    ...
    const handleAddTag = async () => {
        ...
        setSelectedColor('')
        try {
            const response = await upesetTag(subAccountId, tagData)
            toast({
    ...
    const handleDeleteTag = async(tagId : string) => {
        ...
        try {
            ...
            // we should refresh to clear the cash
            router.refresh()
        } catch (error) {
            toast({
                variant : "destructive",
