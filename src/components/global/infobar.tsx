...
const InfoBar = ({notifications, role, className, subAccountId}: Props) => {
    ...
    const handleClick = () => {
        if(!showAll){
            ...
            if(notifications?.length !== 0) {
                setAllNotifications(allNotifications?.filter(item=> item.subAccountId === subAccountId) ?? [])
            }
        }
    ...
    return (
        <>
            <div className={twMerge('fixed z-[20] md:left-[300px] left-0 right-0 top-0 p-4 bg-background/80 backdrop-blur-md flex  gap-4 items-center border-b-[1px]', className)}>
                <div className='flex items-center gap-2 ml-auto'>
                    ...
                    <Sheet>
                        ...
                        <SheetContent className='mt-4 mr-4 pr-4 flex flex-col  overflow-scroll'>
                                    ...
                                    <div className='flex gap-2'>
                                        <Avatar>
                                            <AvatarImage 
                                                ...
                                                alt='Profile Picture'
                                            />
                                            <AvatarFallback className='bg-primary'>{notification.User.name.slice(0, 2)}</AvatarFallback>
                                        </Avatar>
